...
pecosSite <- readNWISsite("08412500") %>% 
...
bayes_specs <- revise(specs = bayes_specs,
                      ...
                      burnin_steps = 1000,
                      saved_steps = 1000)
                      #GPP_daily_mu = 5.427114,
                      #ER_daily_mu = -6.317212,
                      #K600_daily_meanlog = 0.7032425)#  based on smoothed K~Q model
                      #K600_daily_meanlog = 0.5716235, #k based on bayes run
                      #GPP_daily_sigma = 2,
                      #ER_daily_sigma = 2)
                     # other_args = c(max_treedepth = 12)) #try setting prior for k based on smoothed K~Q model
#Run bayes
...
#Save model output
write_csv(pecosBayes, here("ModelOutput/OrlaBayes.csv"))
```
...
ggsave("OrlaBayes_Color.png", bayesFig, here("Figs/"), device = "png", width = 21, height = 15, units = "cm", dpi = "retina")
```
